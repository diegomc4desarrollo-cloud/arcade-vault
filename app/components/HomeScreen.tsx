...
import type { Game, RecentScore, TopPlayer } from "@/app/data";
...
function useReveal() {
  useEffect(() => {
    ...
    if (typeof IntersectionObserver === "undefined") {
      // Sin observer: mostramos todo sin animación (degradación aceptable).
...
function FeatureIcon({ kind }: { kind: FeatureIconKind }) {
  // Iconos pixel 16x16 dibujados con rects, con glow según el color del padre
...
const FEATURES: Array<{ i: FeatureIconKind; t: string; d: string; c: string }> = [
...
export default function HomeScreen({ games, recentScores, topPlayers }: HomeScreenProps) {
  useReveal();
          ...
          <div className="activity-card">
            ...
            <div className="ticker">
              {recentScores.map((r, i) => (
                <div key={`${r.name}-${r.game}`} className="tick-row" style={{ animationDelay: i * 60 + "ms" }}>
