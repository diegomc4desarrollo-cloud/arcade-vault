// Entidades del clon de Asteroids, portadas de
// `references/started-games/02-asteroids/game.js` a TypeScript.
// El canvas interno es fijo 800×600 y el espacio es toroidal (`wrap`).
...
// Estado de entrada que el motor pasa a la nave cada frame (teclado ∪ táctil).
export type ShipInput = {
  left: boolean;
  right: boolean;
  thrust: boolean;
  reverse: boolean; // ↓ propulsión inversa (añadido sobre el original)
};
...
export class Ship {
  ...
  update(dt: number, input: ShipInput) {
    ...
    const REVERSE = 160; // px/s² — retropropulsión, algo más suave
    const DRAG = 0.987;
    ...
    if (input.reverse) {
      this.vx -= Math.cos(this.angle) * REVERSE * dt;
      this.vy -= Math.sin(this.angle) * REVERSE * dt;
    }
